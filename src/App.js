...
import "./App.css";
import timer from "./timer1.gif";
import timeUpImage from "./time-up.png"; // Import the new image
...
const App = () => {
  ...
  const [timerImage, setTimerImage] = useState(timer); // State for the timer image

  useEffect(() => {
    console.log(`Time Left: ${timeLeft}, Game Over: ${gameOver}`);
    if (timeLeft > 0 && !gameOver) {
      ...
      setTimerImage(timeUpImage); // Change the image when time is up
    }else if (timeLeft >= 0){
      setGameOver(true);
      setTimerImage(timeUpImage); 
  ...
  const handleSubmit = (e) => {
    ...
    if (currentGuess.length !== 5 || !isValidWord(currentGuess)) {
      setMessage("Please enter a valid letter's.");
      return;
    }
  ...
  const resetGame = () => {
    ...
    setTimerImage(timer); // Reset the image to the original timer
  };
